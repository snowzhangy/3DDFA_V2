...
def ser_to_obj_multiple2( ver_lst, tri, height, wfp):
    n_obj = len(ver_lst)  # count obj
    ...
    with open(wfp, 'w') as f:
        for i in range(n_obj):
            ...
            for j in range(n_vertex):
                x, y, z = ver[:, j]
                f.write(
                    f'v {x:.2f} {height - y:.2f} {z:.2f} {colors[2]:.2f} {colors[1]:.2f} {colors[0]:.2f}\n')
...
ser_to_obj2 = ser_to_obj_multiple2  # ser_to_obj_multiple
